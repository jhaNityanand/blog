...
@section('css')
@php config(['app.name' => 'Blog | Comment/Add']); @endphp
<!-- Plugin css -->
                ...
                <div class="card-box">
                    <div class="">
                        <form action="{{ route('comment.store') }}" method="post" autocomplete="off">
                            @csrf
                            <input type="hidden" value="{{ Auth::User()->role }}" name="created_by" />

                            <div class="form-group mb-4">
                                <label for="name">UserName</label>
                                <input type="text" value="{{ Auth::User()->name }}" class="form-control @error('name') is-invalid @enderror" name="name" id="name" placeholder="Comment Title" />
                            ...
                            <div class="form-group mb-4">
                                ...
                                <input type="email" value="{{ Auth::User()->email }}" class="form-control @error('email') is-invalid @enderror" name="email" id="email" placeholder="Comment Title" />
                            ...
                            <div class="form-group mb-4">
                                ...
                                <select class="form-control @error('blog_id') is-invalid @enderror select2" name="blog_id" id="blog_id">
                                    ...
                                    <option value="{{ $value->id }}" {{ (old('blog_id') == $value->id) ? 'selected' : '' }}>{{ $value->title }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group mb-4">
                                ...
                                <input type="text" value="{{old('message')}}" class="form-control @error('message') is-invalid @enderror" name="message" id="message" placeholder="Comment Message" />
